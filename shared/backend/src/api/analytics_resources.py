...
from backend.server.model.schemas.analytics import DataSourceCreate, DataSourceUpdate, DataSourceOut
...
def list_resources(
    ...
    auth_utils.require_role(current_user, ["route:analytics#resources"])
...
def get_resource(
    ...
    if "admin" in roles or "route:analytics#datasources#view-password" in roles:
        try:
            data["password"] = auth_utils.decrypt_value(resource.password_encrypted)
        except Exception:
            data["password"] = None
...
def create_resource(
    ...
    auth_utils.require_role(current_user, ["route:analytics#resources#create"])
    ...
    new = DataSource(
        ...
        password_encrypted=auth_utils.encrypt_value(data.password) if data.password else None,
...
def update_resource(
    ...
    auth_utils.require_role(current_user, ["route:analytics#resources#edit"])
    ...
    for key, value in data.dict(exclude_unset=True).items():
        if key == "password" and value:
            setattr(resource, "password_encrypted", auth_utils.encrypt_value(value))
...
def delete_resource(
    ...
    """Delete a resource."""
    auth_utils.require_role(current_user, ["route:analytics#resources#delete"])
